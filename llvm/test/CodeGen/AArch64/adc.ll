...
; RUN: llc -verify-machineinstrs < %s -mtriple=arm64-apple-ios7.0 -global-isel=0 | FileCheck --check-prefixes=CHECK-LE,CHECK-SD-LE %s
; RUN: llc -verify-machineinstrs < %s -mtriple=aarch64_be-none-linux-gnu -global-isel=0 | FileCheck --check-prefixes=CHECK-BE,CHECK-SD-BE %s
; RUN: llc -verify-machineinstrs < %s -mtriple=arm64-apple-ios7.0 -global-isel=1 | FileCheck --check-prefixes=CHECK-LE,CHECK-GI-LE %s
; RUN: llc -verify-machineinstrs < %s -mtriple=aarch64_be-none-linux-gnu -global-isel=1 -global-isel-abort=0  | FileCheck --check-prefixes=CHECK-BE,CHECK-GI-BE %s
...
; CHECK-SD-LE-LABEL: test_imm:
; CHECK-SD-LE:       ; %bb.0:
; CHECK-SD-LE-NEXT:    adds x0, x0, #12
; CHECK-SD-LE-NEXT:    cinc x1, x1, hs
; CHECK-SD-LE-NEXT:    ret
;
; CHECK-SD-BE-LABEL: test_imm:
; CHECK-SD-BE:       // %bb.0:
; CHECK-SD-BE-NEXT:    adds x1, x1, #12
; CHECK-SD-BE-NEXT:    cinc x0, x0, hs
; CHECK-SD-BE-NEXT:    ret
;
; CHECK-GI-LE-LABEL: test_imm:
; CHECK-GI-LE:       ; %bb.0:
; CHECK-GI-LE-NEXT:    adds x0, x0, #12
; CHECK-GI-LE-NEXT:    adc x1, x1, xzr
; CHECK-GI-LE-NEXT:    ret
;
; CHECK-GI-BE-LABEL: test_imm:
; CHECK-GI-BE:       // %bb.0:
; CHECK-GI-BE-NEXT:    adds x1, x1, #12
; CHECK-GI-BE-NEXT:    adc x0, x0, xzr
